...
public class OrderRepository {
    ...
    public boolean putOrderRepo(List<HashMap<String, Integer>> products, Integer userId) {
        ...
        for (HashMap<String, Integer> singleProduct : products) {
            ...
            if (product_stock < quantity) {
                String str = product_id + ":stocks are not sufficient :" + product_stock;
    ...
    public boolean deleteOrderRepo(int orderId) {
        ...
        if (!"PENDING".equals(status)) {
            throw new RuntimeException("only allow you to delete order in pending");
        }

        //2.get backe to stocks
        //2.1get product_id from neworder_detail and quantity list<map<string, integer>>
        //2.2go tp products table and update related stock by using product_id
        ...
        //3.delete order in neworders_detals，and delete related neworders(status) in parent table in DB
    ...
    @Transactional
    public boolean amendentRepo(List<HashMap<String, Integer>> products, int orderId) {
